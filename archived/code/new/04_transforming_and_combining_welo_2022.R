...
unique_states <- unique(c(wimbledon_2022$state)) 
length(unique_states) 

score_importance_dtmc <- as.data.table(read.csv("../data/processed/score_importance_dtmc.csv"))

wimbledon_2022 <- left_join(wimbledon_2022, score_importance_dtmc, by = "state")
colSums(is.na(wimbledon_2022))

#-----------------------------------------------------------------------------------------------------

# divide male & female
wimbledon_2022_male <- wimbledon_2022[1:15567,]
wimbledon_2022_female <- wimbledon_2022[15568:nrow(wimbledon_2022),]
...
# find number of unique entries in player1 and player2 for subset_2022_m
unique_players_2022_m <- unique(c(subset_2022_m$player1_name, subset_2022_m$player2_name)) 
length(unique_players_2022_m) # 73

# make P column in atp_2022_welo_avg all lowercase
atp_2022_welo_avg <- atp_2022_welo_avg %>%
  mutate(P = tolower(P))

# merge atp_2022_welo_avg with subset_2022_m to get the average welo for player1 and player2
subset_2022_m <- subset_2022_m %>%
  left_join(atp_2022_welo_avg, by = c("player1_name" = "P")) %>%
  rename(player1_avg_welo = avg_welo_before_match) %>%
  left_join(atp_2022_welo_avg, by = c("player2_name" = "P")) %>%
...
# remove rows in subset_2022_m with NAs
subset_2022_m <- subset_2022_m %>%
  filter(!is.na(player1_avg_welo) & !is.na(player2_avg_welo))
...
# find average welo for each player in atp_2022_welo_subset
...
length(unique_players_2022_f) # 75
...
# merge atp_2022_welo_avg with subset_2022_m to get the average welo for player1 and player2
subset_2022_f <- subset_2022_f %>%
  left_join(wta_2022_welo_avg, by = c("player1_name" = "P")) %>%
  rename(player1_avg_welo = avg_welo_before_match) %>%
  left_join(wta_2022_welo_avg, by = c("player2_name" = "P")) %>%
  rename(player2_avg_welo = avg_welo_before_match)
...
write.csv(subset_2022_m, "../data/processed/wimbledon_subset_2022_m.csv", row.names = FALSE)
write.csv(subset_2022_f, "../data/processed/wimbledon_subset_2022_f.csv", row.names = FALSE)
